...
export function BottomNav() {
    ...
    return (
        <div className="fixed bottom-0 left-0 right-0 z-50 flex justify-center pb-safe">
            <nav className="w-full max-w-md glass px-6 py-3 flex items-center justify-between rounded-t-3xl shadow-[0_-10px_40px_rgba(0,0,0,0.5)]">
...
function NavItem({ href, icon, label, active }: { href: string; icon: React.ReactNode; label: string; active?: boolean }) {
    return (
        <Link href={href} className="flex flex-col items-center gap-1.5 w-16 group outline-none">
            <div className={`p-2 rounded-xl transition-all duration-300 ${active ? 'bg-primary/20 text-primary shadow-sm' : 'text-foreground/40 hover:text-foreground/70 group-active:scale-95'}`}>
                {icon}
            </div>
            <span className={`text-[10px] font-bold transition-colors ${active ? 'text-primary' : 'text-foreground/40 group-hover:text-foreground/70'}`}>
                {label}
            </span>
        </Link>
    )
